...
class miDuplicador extends HTMLElement {
 ...
 constructor() {
        super(); // Siempre llamar a super() en el constructor
        this.duplicador = parseInt(this.getAttribute('duplicador')) || 0;
      }
      ...
      // Se ejecuta cuando se cambia un atributo observado
      attributeChangedCallback(nombre, valorAntiguo, valorNuevo) {
        if (nombre === 'duplicador') {
          this.duplicador = parseInt(valorNuevo);
          this.render();
        }
      }
        ...
        // Método para Multiplicar
      multiplicar = () => {
        this.duplicador = this.duplicador * 2;
        this.setAttribute('duplicador', this.duplicador);
      }

      // Método para actualizar el contenido del elemento
      render() {
        this.textContent = `Contador: ${this.duplicador}`;
      }
...
customElements.define('mi-tabla', MiTabla);
    ...
    customElements.define('mi-duplicador', miDuplicador);
